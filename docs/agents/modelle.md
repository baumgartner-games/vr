...
| Scherben, Deko-Scheibe | — | `prototype-bits/target_pieces_A–F.glb`, `medieval-hexagon/decoration/props/target.glb` (0,24 × 0,30 × 0,14 Quelle → **0,12 × 0,15 × 0,07 m**) | **in der zweiten Runde genommen** | Die Scherben sind das, was von einer zerschossenen Scheibe übrig bleibt — dafür gab es hier nichts, **bis es das Zerschießen gab** (siehe unten). Die Sechseck-Scheibe ist 15 cm hoch: Tischdekoration, keine Zielscheibe. |
| **Straßenlampe** (Mast, Ausleger, Haube, Glas, `PointLight`) | `worlds/grid/fixtures/lamp.ts` `LAMP.build`; aufgestellt in `zones/interact.ts` und `zones/podium.ts`, beide mit `height: 3.2` | `city-builder-bits/streetlight.glb` — 0,269 × 0,960 × 0,069 Quelle → **0,13 × 0,48 × 0,03 m** | **bleibt liegen**; ersetzt wurde durch ein anderes Stück (siehe unten) | Die Bauart stimmt (Mast, Ausleger, Kopf), die **Verhältnisse** nicht: Der Ausleger des Modells misst 0,239 von 0,960, also **25 %** seiner Höhe; der gebaute misst 0,30 m bei 3,20 m, also **10 %**. Auf 3,20 m gebracht (×6,7) ragt der Kopf 0,80 m vom Mast — einen halben Meter neben das Licht, das in der Kachelmitte hängt. Rückt man den Mast unter den Kopf, steht er auf der **Nachbarkachel**, und der Mast steht mit Absicht am Rand („Ein Mast mitten auf der Kachel stünde genau dort, wo man stehen will"). |
...
Die Tabelle oben ist eine **Bestandsaufnahme** und war nie ein Urteil. Sie
sagt, was gemessen dabei herauskam, und sie hat an mehreren Stellen „bleibt
gebaut" gesagt, weil das Modell im Regal die gebaute Form nicht eins zu eins
traf. Der Auftraggeber hat sie gelesen und anders entschieden: Lampen,
Schildpfosten, das Stachelfeld, die Regale, die Zielscheibe samt Ständer und
die Hebel sollen aus dem Regal kommen, die alten Formen sollen **weg**, und
der Boden draußen soll ein Plattenboden werden.

Das ist keine Korrektur der Messungen — die stimmen alle noch —, sondern eine
andere Abwägung derselben Zahlen: Ein Stück, das aussieht wie von Hand
gezeichnet, ist mehr wert als eines, dessen Ausleger im richtigen Verhältnis
steht. Die **drei Regeln** oben gelten unverändert und haben jeden dieser acht
Tausche getragen; Regel 1 („Ersetzt wird nur das Bild, nie die Rechnung") ist
dabei kein einziges Mal gebrochen worden.

| Ding | Wo gebaut | Modell | Wie eingepasst |
| --- | --- | --- | --- |
| **Lampe** | `grid/fixtures/lamp.ts` | `holiday-bits/lantern.glb` (3,938 Quelle → 1,97 m) | Auf `lampHeight(place)` gemessen (`lampFit`). Das Leuchtteil wird über den **Materialnamen** `holiday_glow` gefunden — KayKit markiert es selbst mit `emissiveFactor` —, und `apply` färbt es zwischen an und aus. Mast, Ausleger, Haube und Glaskugel sind ersatzlos weg; geblieben sind das `PointLight` und ein unsichtbarer Stellvertreter als Griff (`GridWorld.attachUsable` meldet **synchron beim Bauen** an, da ist die Datei noch unterwegs). |
| **Schildpfosten** | `grid/fixtures/sign.ts` (1,35 m), `signs/SignBoard.ts` (1,05 m), `zones/navigation.ts` (1,80 m), `zones/range.ts` (1,50 m) | `dungeon/post.glb` (4,000 Quelle → 2,00 m) | Vier Stellen, ein Modell, vier Höhen — genau die Naht, für die es jetzt `core/kaykitHeight.ts` gibt: `kaykitHeightScale` als reine Rechnung, `kaykitFitHeight` misst und stellt den **Fuß** auf den Ursprung, `kaykitAtHeight` bringt Schranke und dynamischen Import in eine Zeile. |
| **Stachelfeld** | `zones/navigation.ts` `buildSpikes` | `platformer/red/floor_spikes_trap_2x2x1_red.glb` (2,000 Quelle → 1,00 m) | Neun Kopien, eine je Kachel von `SPIKES`. Die Datei hat **zwei** Knoten — Platte und Stacheln getrennt —, und genau daran wird gemessen: Der Stachelknoten wird zum Messen kurz abgehängt, die Oberkante der Platte kommt auf den Boden, die Spitzen ragen 25 cm heraus. `plan.floor(…, { hazard: HAZARD_SPIKES })` ist unberührt: Die Gefahr steht im Graphen, nicht im Bild. |
| **Regal** (Baustein `shelf`) | `grid/blocks.ts` `BUILD.shelf`, gesetzt in `kart/kartPit.ts`, `zones/range.ts`, `editor/starterGrid.ts` | `dungeon/bookcase_single.glb` (2,000 × 3,000 × 0,500 Quelle → 1,00 × 1,50 × 0,25 m) | Der erste **Baustein**, der ein Modell bekommt, und deshalb der aufwendigste Tausch: Ein Baustein wird zu `PlanSolid`-Quadern plattgedrückt und weiß danach nicht mehr, dass er ein Regal war. Also trägt der Quader seine Herkunft mit (`PlanSolid.block`), `GridWorld` findet die Quader einer Kachel darüber wieder, schaltet sie **unsichtbar** (der Körper trägt weiter die Kollision, der Graph weiter seine Kosten) und hängt das Modell daneben. Sie dürfen dafür in **kein** Bündel (`gridBatch.joinsBatch`) — ein unsichtbarer Quader in einem `InstancedMesh` ist wieder sichtbar. `BLOCKS.shelf.height` ist dabei von 1,9 auf **1,5** gegangen: Genau dann ist der Maßstab 1, und gebautes Regal, Körper, Wegekosten und Modell sind dieselbe Kiste. |
| **Zielscheibe** | `zones/range.ts` `spawnTarget` | `prototype-bits/target.glb` (→ 0,70 m Ø) | Die Absage oben galt den **Ringen**: Das Modell hat vier, gezählt wird nach fünf (`range/scoring.RINGS`). Genommen wurde es trotzdem, und die Absage ist damit eingelöst statt übergangen — `faceHit` rechnet unverändert weiter, nur das sichtbare Netz ist getauscht. Wer in den zweiten Ring trifft, bekommt weiterhin, was die Rechnung sagt, und nicht, was das Bild zeigt; das ist der Preis und er steht hier. `TARGET_R` ist mit dem Modell von 0,30 auf 0,35 gegangen — überall zugleich: Gelenkpunkt, Hülle, Masse, Wertung. |
...
hätte das die **geteilte** Geometrie allen anderen Kopien und der Vorlage
