...
| Haunting: Schalttafel | – | Tür oder Lampe antippen schaltet sie, Zimmer antippen den Schallköder | dito |
...
- **Die drei Nicht-VR-Rollen sind Karten** (`views/`), und zwar dieselbe
  ...
  **nicht** sieht, steht deshalb nicht in einem Kommentar, sondern in ihren
  ...
  - **Schalttafel** (`views/panelRole.ts`, Kennung `hack`): der Grundriss
    ...
    Licht, ein Tipp auf ein **Zimmer** wirft dessen Schallköder an — der hat
    kein Symbol im Bild, weil er ein Lautsprecher an der Decke ist. Geschaltet
...
- **Rollenwechsel in der 2D-Welt** (`views/roleStrip.ts`): ein Streifen über
  ...
  `FlatMode.roleHost()`; die Schalttafel greift über `FlatRound.lockDoor`,
  `switchLight` und `lure` in dieselbe Runde.
