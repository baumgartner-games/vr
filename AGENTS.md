  ...
  **Norden ist oben und die Mitte ist der Startplatz.** Die Himmelsrichtung ist
  ...
  - **Küche** (ganz im Norden, hinter dem Podest): vierundzwanzig mal elf
    ...
    **Schauraum**: jedes der achtzehn Möbel noch einmal, frei stehend und mit
    ...
    in dem sich leere Möbel versetzen lassen — das Einschalten räumt die Küche
    dabei ab, wie `B`/`Y` es täte (beides unter _Anfassen in der
    Küche_). Sie ist
...
| Küche: umbauen | roter Knopf neben dem Eingang + `A` schaltet den Baumodus um (sein Schild sagt, wohin: _Küche umbauen_ / _Küche nutzen_); das Einschalten räumt die Küche ab wie `B`/`Y`, dann `A` am leeren Möbel hebt es auf, `A` auf dem Umriss davor setzt es ab (grün = passt, rot = passt nicht) | dito mit `E` | dito mit `A` | dito mit Knopf `A` |
...
- **Der Baumodus hängt an einem Knopf in der Küche** (`zones/kitchenBuild.ts`).
  ...
  Schritt um zwei Felder.
  **Der Umbau fängt in einer abgeräumten Küche an** (`calmStations`), und das
  Anschalten macht damit dasselbe wie `B`/`Y`: alle Uhren aus, alle Flächen
  leer, Geräte zurück an ihren Platz, der Gast vom Tisch, die Bänder halten.
  Bis eben warf es nur weg, was in der Hand lag, und ließ alles andere laufen.
  Zwei Gründe, und jeder allein reicht. Ein Herd, der brennt, während man das
  Möbel daneben verrückt, ist kein Bauzustand, sondern ein Unfall mit einem
  Zeitlimit — man baut um, man kocht nicht. Und aufheben ließ sich ohnehin
  nichts, worauf noch etwas stand (`liftPiece`): Man schaltete den Umbau ein und
  musste erst einmal hinterherräumen, bevor man das erste Möbel anfassen
  durfte.
  **Sein eigenes Gerät ist von „leer" ausgenommen** (`ownGear`, `stowGear`),
  und ohne diese Ausnahme wäre die Regel eine Sperre: Topf, Pfanne und
  Feuerlöscher stehen auf **ihrem** Möbel, dort gehören sie hin, und dorthin
  stellt sie auch jedes Abräumen zurück. Zählte man sie mit, hätte die Küche
  drei Möbel, die sich nie versetzen lassen — und keinen Weg, sie leer zu
  bekommen, denn im Umbau meint `A` das Möbel und nicht mehr den Topf darauf.
  Also **reist das Gerät mit**: Es hängt sich unter das Netz des Möbels (mit
  gegengerechnetem Maßstab, sonst erbt es dessen Halbierung) und steht beim
  Absetzen wieder auf dessen Ablage. Erkannt wird es am **Heimweg**
  (`Carried.home`) und nicht am Namen — ein Topf, den jemand auf einem fremden
  Tisch abgestellt hat, ist dort keiner, und dieser Tisch bleibt stehen.
  **Ein Möbel in der Hand arbeitet dabei nicht** (`cook` übergeht es): Ein
  Patty, das vor dem Bauch weiterbrutzelt und unterwegs Feuer fängt, ist kein
  Spiel, sondern ein Rätsel. Seine Anzeigen gehen beim Aufheben mit aus; ein
  Balken, der dort weiter schwebt, wo das Möbel einmal stand, ist die Auskunft
  über eine Station, die es an dieser Stelle nicht mehr gibt.
  **Und das Umschalten macht in beide Richtungen die Hände frei.** Beim
  Ausschalten war das schon so; beim Einschalten ist es nötig, seit ein Möbel
  auch ohne Umbau in die Hand kommt (`takeFromCatalogue`) — ein getragener Herd
  wäre sonst eine Station, die das Abräumen gleich darauf auf ihre alte Kachel
  zurückräumt, und der Topf läge dort, wo der Herd einmal stand.
  **Und die Geräte gehen heim, statt weggeworfen zu werden.** Was beim
  Anschalten in der Hand lag, wurde bis eben **entsorgt** — auch Topf, Pfanne
  und Feuerlöscher, und die gibt es genau einmal in dieser Küche
  (`core/kitchenModel.takeUtensil`). Wer mit der Pfanne in der Hand umzubauen
  anfing, hatte danach keine mehr und keinen Weg, eine neue zu bekommen. Essen
  wird weggeworfen, Gerät geht an seinen Platz — und zwar **leer**, ohne seinen
  Belag: ein Patty, das in der zurückgestellten Pfanne weiterbrutzelt, wäre kein
  Aufräumen. Dass auch das Schild dabei mitwechselt, steht in `showBuildLabel`
  und an **einer** Stelle: `reset()` legte `editing` um, ohne die Beschriftung
  mitzudrehen, und dann stand nach dem Aufräumen weiter _Küche nutzen_ auf dem
  Knopf, während längst wieder gekocht wurde. Eine Beschriftung, die das
  Gegenteil dessen sagt, was der nächste Druck tut, ist schlimmer als gar keine.
