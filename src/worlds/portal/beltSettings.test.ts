...
describe('Gürtel-Position', () => {
  it('liefert den Gürtel auf Hosenbundhöhe aus', () => {
    expect(DEFAULT_BELT).toEqual({ side: 0.26, height: 0.825, forward: -0.04 });
  });
  ...
  it('hängt die Hüften auf halbem Weg zwischen 0,65 der Augenhöhe und den Augen auf', () => {
    expect(beltSlotPoint(DEFAULT_BELT, 'right', HEAD, 0).y).toBeCloseTo(1.32, 6);
    // Wer sitzt, hat den Gürtel tiefer — die Höhe ist ein Anteil, keine Zahl.
    expect(beltSlotPoint(DEFAULT_BELT, 'right', 1.2, 0).y).toBeCloseTo(0.99, 6);
  });
  ...
  it('rechnet gezogene Zentimeter in Anteile der Augenhöhe um', () => {
    ...
    expect(raised.height).toBeCloseTo(0.925, 6);
    // Dieselbe Höhe soll auch dort herauskommen, wo sie gezogen wurde.
    expect(beltSlotPoint(raised, 'right', HEAD, 0).y).toBeCloseTo(1.48, 6);
  });
  ...
  it('schreibt die drei Zahlen so auf, wie sie in der Brille zu lesen sind', () => {
    expect(beltLabel(DEFAULT_BELT)).toBe('26 cm · 0.825 · 4 cm hinten');
