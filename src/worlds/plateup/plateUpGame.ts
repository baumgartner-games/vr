...
export function freeTables(shift: Shift, tables: number): number[] {
  ...
  const out: number[] = [];
  for (let i = 0; i < tables; i++) if (!taken.has(i)) out.push(i);
  return out;
}
...
export function stepShift(shift: Shift, dt: number, tables: number): ShiftTick {
  ...
  for (const guest of shift.guests) {
    if (guest.phase === 'waiting') {
      const patience = guest.patience - step;
      if (patience <= 0) {
  ...
  if (lost >= rules.lives) {
    ...
    return {
      shift: { ...shift, phase: 'over', time, guests, lost, served, coins, total, seed },
      events,
    };
  }
  ...
  if (phase === 'open') {
    ...
    const free = freeTables({ ...shift, guests }, tables);
...
export function serveTable(shift: Shift, table: number, d: Dish | null): ServeResult {
  const guest = guestAt(shift, table);
  if (!guest) return { ok: false, why: 'An diesem Tisch sitzt niemand' };
  if (guest.phase === 'walkIn') return { ok: false, why: 'Der Gast setzt sich gerade erst' };
  if (guest.phase === 'eating') return { ok: false, why: 'Hier wird schon gegessen' };
  const wanted = menuItem(guest.order);
  const recipe = plateRecipe(d);
  if (!d) return { ok: false, why: `Bestellt: ${wanted.label}` };
  if (d.item !== 'plate') return { ok: false, why: 'Serviert wird auf einem Teller' };
  if (!recipe) return { ok: false, why: `Das ist noch kein ${wanted.label}` };
  if (recipe.id !== guest.order) {
    return { ok: false, why: `Falscher Burger — bestellt war ${wanted.label}` };
  }
