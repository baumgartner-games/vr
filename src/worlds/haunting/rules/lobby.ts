/**
 * **Was ich vorhabe — und wer ich dabei bin.**
 *
 ...
 * gerade tut. Hier steht sie einmal:
 *
 ...
 * - **Wer ich bin** (`me`) — ein Platz oder ein Zuschauer, je Gerät.
 *
 * **Eine Ansicht gibt es hier nicht mehr.** Bis September 2026 stand daneben
 * `view: '2d' | '3d'` — die gemalte Karte von oben oder das Schiff. Die
 * Karte ist weg (Plan H, `docs/plan-haunting-1m.md`): Von oben sieht man
 * seitdem dieselbe Szene wie in der Brille, aus der Kamera des Kerns, und
 * das ist keine Wahl der Lobby, sondern _Menü → Ansicht_ wie in jeder Welt.
 * Ein gemerkter Stand mit `view` wird gelesen und das Feld dabei vergessen.
 *
 * Reine Daten, ohne DOM und ohne three.js: Die Lobby im Van und das Menü in
 * der Brille lesen und schreiben dieselbe Wahl, gemerkt im Browser
 * (`LOBBY_STORAGE`).
 */
 ...
 * sitzt man in der Zentrale und sieht dem Techniker zu; am Desktop und in der
 * Brille trägt man den Anzug, denn dort gibt es das Schiff, und ein Schiff
 * ohne Techniker ist eine Vorführung.
 */
 ...
 * Techniker, und das Schiff; „2D Einsatzzentrale" ist jeder andere Platz —
 * wer dort schon einen hatte (Rot, Monster, ein Zuschauer), behält ihn, und
 * nur wer als Techniker gemerkt war, fängt so an wie jedes Telefon: als
 * Zuschauer des Technikers (`defaultLobby`).
 */
export function arriveAs(choice: LobbyChoice, entry: Entry): LobbyChoice {
  if (entry === 'technician') return { ...choice, me: 'technician' };
  return { ...choice, me: choice.me === 'technician' ? 'watch:technician' : choice.me };
}

/**
 * Aus fremdem Text (Speicher) eine gültige Wahl — Unbekanntes wird ersetzt,
 * und ein altes Feld `view` wird nicht mehr gelesen.
 */
export function readLobby(value: unknown, role: PlayerRole = 'desktop'): LobbyChoice {
...
export function loadLobby(
  ...
  try {
    ...
    return readLobby(raw ? JSON.parse(raw) : null, role);
  } catch {
