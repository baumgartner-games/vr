...
import { isPassage, onApron, roomAt, type HouseDoor, type HouseSpec } from './house';
...
/**
 * **Was man von oben sieht: den eigenen Raum und ganze Nachbarräume hinter
 * offenen Türen** (Wunsch des Besitzers, September 2026: _„Eigener Raum und
 * der andere Raum, wo die Tür geöffnet ist, aber sauber eben"_).
 *
 * Die Brille fragt, ob eine Türöffnung im Blickfeld liegt
 * (`HauntingWorld.cullRoomArt`) — von oben liegt aber *jede* Tür im Bild, und
 * die Kamera schaut über alle Wände hinweg in die Nachbarräume. Von oben zählt
 * deshalb nicht die Kamera, sondern die Figur:
 *
 * - Ihr Raum ist immer zu sehen — samt allen Gangstücken, die über offene
 *   Durchgänge (`HouseDoor.passage`) daran hängen: Ein Gang ist ein Raum, auch
 *   wenn er aus mehreren Stücken besteht.
 * - Ein Nachbar ist zu sehen, **solange die Tür dorthin offen steht** (`open`,
 *   in der Welt das Blatt der Türautomatik) und nicht verriegelt ist (`shut`)
 *   — dann ganz, samt seinen Gangstücken. Früher entschied ein Abstand
 *   (6 m zur Türmitte), und der Nachbar lag angeschnitten da.
 * - Weiter nicht: Der Raum hinter dem Nachbarn bleibt dunkel, auch wenn
 *   dessen nächste Tür offen steht.
 *
 * Alles andere deckt die Welt zu (`world3d/topDownFog.ts`). `null` heißt wie
 * oben: Die Figur steht in keinem Raum der Station, und es bleibt alles
 * sichtbar.
 */
export function topDownRooms(
  ...
  viewer: { x: number; z: number },
  shut: readonly string[],
  open: (door: HouseDoor) => boolean,
): Set<string> | null {
  const room = roomAt(spec, Math.floor(viewer.x / TILE), Math.floor(viewer.z / TILE));
  if (!room) return null;
  const through = (door: HouseDoor): boolean =>
    !!door.b && !shut.includes(door.id) && (isPassage(door) || open(door));
  // Ein Raum samt den Gangstücken, die über Durchgänge daran hängen.
  const whole = (id: string, into: Set<string>): void => {
    const queue = [id];
    into.add(id);
    for (let index = 0; index < queue.length; index++) {
      const from = queue[index]!;
      for (const door of spec.doors) {
        if (!isPassage(door) || !through(door)) continue;
        const next = door.a === from ? door.b : door.b === from ? door.a : null;
        if (!next || into.has(next)) continue;
        into.add(next);
        queue.push(next);
      }
    }
  ...
  for (const door of spec.doors) {
    if (isPassage(door) || !through(door)) continue;
    const next = own.has(door.a) ? door.b! : own.has(door.b!) ? door.a : null;
    if (next && !visible.has(next)) whole(next, visible);
  }
  return visible;
}
